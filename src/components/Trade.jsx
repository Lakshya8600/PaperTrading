  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-950 text-white px-4 py-12">
        ...
        {/* Active Trades */}
        <div className="mt-12">
          <h2 className="text-2xl font-bold text-center text-green-400 mb-4">Active Trades</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {activeBuy.map((trade, index) => (
              <div key={index} className="bg-gray-800 p-4 rounded-lg shadow-md">
                <h3 className="text-lg font-bold text-green-400">Buy Trade</h3>
                <p>Stock: {trade.stock}</p>
                <p>Quantity: {trade.quantity}</p>
                <p>Price: ₹{trade.price}</p>
                <button
                  className="mt-4 px-4 py-2 bg-red-500 text-white rounded-lg"
                  onClick={() => handleCloseTrade(index, "buy")}
                >
                  Close Trade
                </button>
              </div>
            ...
            {activeSell.map((trade, index) => (
              <div key={index} className="bg-gray-800 p-4 rounded-lg shadow-md">
                <h3 className="text-lg font-bold text-red-400">Sell Trade</h3>
                <p>Stock: {trade.stock}</p>
                <p>Quantity: {trade.quantity}</p>
                <p>Price: ₹{trade.price}</p>
                <button
                  className="mt-4 px-4 py-2 bg-red-500 text-white rounded-lg"
                  onClick={() => handleCloseTrade(index, "sell")}
                >
                  Close Trade
                </button>
              </div>
